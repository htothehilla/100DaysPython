...
#the map() function has two required inputs: map(function, iterative object).
...
#The filter() function is similar to the map() function where a function
# is applied to each item in the iterable object.
# However, filter() is used to only return items that return the True response.
# Additionally, the filter() function creates a filter object, so the list() function is required to store the output in a list.
# In order to understand why lambda i: i % 2 provides a list of odd values, execute print(int(True)).
...
#Similar to compressing a folder into a .zip file, the zip() function compresses
# multiple iterable objects into a single object. The output of the zip() function is a
# zip object. Therefore, the output from the zip() function can be saved as a list with the list() function.

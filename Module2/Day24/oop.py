# object orineting programming can be used for application development
# OOP primarily revolves around Classes, Functions, and Decorators.

#A decorator is a function that leverages another function without changing it.
...
foo(bar)
# simple decorator
# @ Similar to the lambda function, the "pie" syntax is an effective method for eliminating extraneous code, but it can reduce the overall readability of the program
#The "pie" syntax can be used to declare the function being called as the declarator,
#followed by the function used as an input to the declarator.
...
#CLASES
#classes are an invaluable aspect of OOP that leads to enhanced functionality while retaining readability

#According to PEP8, the class name should be in camel case and does not require parentheses
# if no arguments are provided. Additionally, a class requires an action to be created, so the pass command is often used to initially
# create the framework of the class before the underlying functions are created.
...
#value of classes over functions
